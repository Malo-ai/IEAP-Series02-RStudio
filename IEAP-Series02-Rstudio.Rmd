...
# --- Fonction utile pour choix test t-test ou Wilcoxon ---
run_test_numeric <- function(var, group) {
  tbl <- table(group)
  
  # Si taille de groupe <3, test non-paramétrique par défaut
  if (any(tbl < 3)) {
    cat("Taille de groupe trop petite, utilisation Wilcoxon\n")
...
# --- Fonction pour test qualitatif χ² ou Fisher ---
...
# --- Q1 : Poids selon ronflement ---
cat("\nQ1 : Poids selon ronflement\n")
res1 <- run_test_numeric(snore$weight, snore$snore)
print(res1)
boxplot(weight ~ snore, data=snore, main="Poids selon ronflement",
        ylab="Poids (kg)", xlab="Ronflement")

# --- Q2 : Alcool selon ronflement ---
cat("\nQ2 : Alcool selon ronflement\n")
res2 <- run_test_numeric(snore$alcool, snore$snore)
print(res2)
boxplot(alcool ~ snore, data=snore, main="Alcool selon ronflement",
        ylab="Verres/semaine", xlab="Ronflement")

# --- Q3 : Tabac selon ronflement ---
cat("\nQ3 : Tabac selon ronflement\n")
res3 <- run_test_categorical(snore$tabac, snore$snore)
print(res3)
barplot(table(snore$tabac, snore$snore), beside=TRUE, legend=TRUE,
        main="Tabac selon ronflement")

# --- Q4 : Poids selon sexe ---
cat("\nQ4 : Poids selon sexe\n")
res4 <- run_test_numeric(snore$weight, snore$sex)
print(res4)
boxplot(weight ~ sex, data=snore, main="Poids selon sexe",
...
print(res5)
barplot(table(snore$tabac, snore$sex), beside=TRUE, legend=TRUE,
        main="Tabac selon sexe")

# --- Corrélations générales (variables numériques) ---
...
Poids selon ronflement
« Les individus qui ronflent avaient un poids légèrement plus élevé que ceux qui ne ronflent pas. Le test statistique approprié (t-test ou Wilcoxon selon conditions) a montré que cette différence était [significative/non significative], p = … »

Alcool selon ronflement
« La consommation d’alcool ne différait pas significativement entre les ronfleurs et les non-ronfleurs (p = …). »

Tabac selon ronflement
« Les ronfleurs étaient plus susceptibles d’être fumeurs que les non-ronfleurs. Le test χ²/Fisher a montré une différence significative, p = … »

Poids selon sexe
« Les hommes avaient un poids moyen plus élevé que les femmes. Le test statistique a confirmé que la différence était significative, p = … »

Tabac selon sexe
« Les femmes fumaient moins fréquemment que les hommes. Le test χ²/Fisher a montré une différence significative, p = … »

Corrélations générales
« Une analyse de corrélation (Spearman) a révélé que le poids était positivement corrélé avec la consommation d’alcool et avec le ronflement, indiquant que certains facteurs de style de vie peuvent être liés entre eux. »

Répondre à la question : do we have any correlation beetween variable ? 
IDK for the moment because i go to librarie to answer and have a complete relecture 
